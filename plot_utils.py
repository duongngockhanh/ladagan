...
def save_generator_heads(model, epoch, noise, main_dir, 
    ...
    with torch.no_grad():
        predictions, maps = model(noise)
    predictions = np.clip(deprocess(predictions.cpu().numpy()), 0, 255).astype(np.uint8)
    ...
    for r in range(n_resolutions):
        for h in range(heads[r]):
            ...
            for i in range(predictions.shape[0]):
                fig.add_subplot(8, 8, i+1)
                map_reshape = maps[r][i][h].reshape(map_size, map_size).cpu()
...
def get_map(maps, resolution, head):
    ...
    map_size = int(math.sqrt(maps[r][0][0].shape[0])) # get map high and width 
    b = maps[r][:, h].shape[0]
    reshaped_maps = maps[r][:, h].reshape(b, map_size, map_size, 1)
...
def plot_single_head(predictions, maps, h=1, size=1, path=None):
    ...
    for i in range(n):
        axes[0, i].imshow(predictions[i].cpu())
        axes[0, i].axis('off')

    # Plot maps32 in the second row
    for i in range(n):
        axes[1, i].imshow(maps32[i, :, :, 0].cpu())
        axes[1, i].axis('off')

    # Plot maps16 in the third row
    for i in range(n):
        axes[2, i].imshow(maps16[i, :, :, 0].cpu())
        axes[2, i].axis('off')

    # Plot maps8 in the fourth row
    for i in range(n):
        axes[3, i].imshow(maps8[i, :, :, 0].cpu())
        axes[3, i].axis('off')
